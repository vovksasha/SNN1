    class KDTreeNode {
        ...
        function visualize3D(points, targetPoint, nearestPoints) {
            ...
            nearestPoints.forEach(p => {
                const nearestGeometry = new THREE.SphereGeometry(1);
                const nearestMaterial = new THREE.MeshBasicMaterial({ color: 0x00ff00 });
                const nearestSphere = new THREE.Mesh(nearestGeometry, nearestMaterial);
                nearestSphere.position.set(p[0], p[1], p[2]);
